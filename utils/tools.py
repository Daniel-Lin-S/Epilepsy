...
def get_patients_with_seizures(csv_file: str):
    """
    Read a CSV file and extract Patient_IDs where Seizure_Count > 0. \n
    `patient_summary.csv` will be generated by `annotation.py`
    ...
    -------
    list
        List of Patient_IDs with Seizure_Count > 0.
    int
    ...
    # Filter rows where Seizure_Count > 0
    filtered_df = df[df['Seizure_Count'] > 0]

    # Extract the Patient_ID column from the filtered DataFrame
    patient_ids = filtered_df['Patient_ID'].tolist()

    n_seizures = filtered_df['Seizure_Count'].sum()

    return patient_ids, n_seizures
